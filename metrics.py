...
# method to get the bertscores between a list of backtranslations and a list of originals (paired)
def get_bertscore(candidates, references, lang="en"):
    bertscore = evaluate.load("bertscore")
    return bertscore.compute(predictions=candidates, references=references, lang=lang)
    ...
    # if the two parts are of different length, remove lines iteratively
        ...
        while diff > 0:
            ...
            for x in range(len(metre)):
                value = sum(metre[x]) + sum(
                    [min([table[i][j] for i in range(diff)]) for j in range(len(stresses))]) / len(stresses)
                if value < min_value:
                    min_value = value
                    min_index = x
